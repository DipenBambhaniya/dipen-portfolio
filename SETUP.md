...
Instructions for running, editing, and deploying this site. For what this
...
`/resume` ([`src/app/resume/page.tsx`](src/app/resume/page.tsx)) renders the
same content as `src/content/profile.ts` as a web page, with a button to
download the PDF at [`public/resume.pdf`](public/). The phone number shown on
that page is masked via [`src/lib/maskPhone.ts`](src/lib/maskPhone.ts); the
full number still appears in the Contact section and the PDF itself.
