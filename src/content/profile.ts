    ...
    roles: [
      {
        ...
        highlights: [
          ...
          'Added PayPal as a checkout option on top of the existing Spreedly integration — Spreedly routes the purchase through Braintree as the gateway, which settles with PayPal, so the same purchase/refund pipeline built for cards now also powers PayPal orders on trulyfree.com and Truly-Free Home (TFH), which places its orders through the same Truly-Free payment APIs.',
...
export const projects: Project[] = [
  {
    ...
    blurb:
      'Payment microservices built on Spreedly: PCI-safe card capture through a Spreedly iFrame, vendor-token exchange, and server-side charging for checkout, settlement, and refunds. Added Apple Pay and Google Pay wallet checkout and a Spreedly Account Updater flow to keep stored cards alive, with Avalara for real-time US sales-tax across jurisdictions. Most recently added PayPal by routing Spreedly through Braintree as the gateway, which settles with PayPal — reusing the same purchase/refund pipeline to also power orders for Truly-Free Home (TFH). Built for correctness under retry and partial failure.',
